...
def test_adding_new_lookup_json_works(tmp_scratch_disk):
    ...
    assert len(us2.ingested_credentials) == 3
...
def test_adding_duplicate_is_overwriting_old(tmp_scratch_disk):
    ...
    os.environ["U_MINIO_USER"] = "Mitsurugi"
    os.environ["U_MINIO_PASSWORD"] = "+==|----->"

    os.environ["M_USER"] = "Horst"
    os.environ["M_PASSWORD"] = "Walter"
    us = urgap.UCredentialManager(json_path=c_json)

    assert us.get_user("minio-libcloud://localhost:9000") == "Mitsurugi"
    assert len(us.ingested_credentials) == 2

    us.add_credentials(standard_lookup)

    assert us.get_user("minio-libcloud://localhost:9000") == "Horst"
    assert len(us.ingested_credentials) == 2
...
def test_env_extraction_works(tmp_scratch_disk):
    ...
    us = urgap.UCredentialManager(json_path=c_json)
    us.add_credentials(standard_lookup)
    assert us.get_user(standard_lookup[0]) == "Horst"
    assert us.get_password(standard_lookup[0]) == "Walter"
    del os.environ["M_USER"]
    del os.environ["M_PASSWORD"]

    assert us.get_user(us.ID_KEY.format(**standard_lookup[0])) == "Horst"
...
def test_null_user_works():
    ...
    us = urgap.UCredentialManager(json_path=c_json)
    us.add_credentials(standard_lookup)
    assert us.get_user(standard_lookup[0]) == None
    assert us.get_password(standard_lookup[0]) == "Walter"
    del os.environ["M_PASSWORD"]
    assert us.get_user(us.ID_KEY.format(**standard_lookup[0])) == None
