...
def test_rebase_success():
    ...
    ucredentials = [{"user": "test"}]
    config = {"param": "value"}
    uris = ["dummy_uri1", "dummy_uri2"]
    storage_base_uri = "s3://bucket/base/"

    mock_ufile = MagicMock()
    mock_ufile_list = [mock_ufile, mock_ufile]

    with (
        patch("urgap.uhelpers.prefect.setup_urgap") as mock_setup,
        patch(
            "urgap.uhelpers.prefect.retrieve_processed_uris", return_value=uris
        ) as mock_retrieve,
        patch(
            "urgap.uhelpers.prefect.urgap.UFileList.from_uri_list",
            return_value=mock_ufile_list,
        ),
    ):
        result = rebase(
            uris=uris,
    ...
    for uf in mock_ufile_list:
        uf.rebase.assert_called_with(uri=storage_base_uri, upload=True)
