...
def run_server(name: str, port: int, shutdown_event: multiprocessing.Event) -> None:
...
def send_signal_to_pid(sig: int = signal.SIGINT) -> None:
    """Send the specified signal to the parent PID."""
    pid = os.getppid()
    os.kill(pid, sig)
    logger.info("Sent signal %s to process %s", sig, pid)
...
def upi_server(nodes: tuple | str, mcp: bool | None) -> None:
    """Spawn servers for requested Urgap nodes.

    Args:
        nodes: Node names/versions to serve. Multiple can be passed.
        mcp: Flag whether or not to spawn mcp server.

    This spawns a process for each node/version, each on its own port.
    """
    processes = []
    shutdown_event = multiprocessing.Event()
